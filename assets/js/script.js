...
// ==========================================
// 🚀 MC PRODUCTIONS - CORE SCRIPT OPTIMIZADO
// ==========================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mostrarAlerta = (mensaje, tipo = "normal") => {
        const texto = document.getElementById('texto-notificacion');
        texto.textContent = mensaje;
        notificacion.style.backgroundColor = tipo === "error" ? "#ff4d4d" : "var(--color-primario)";
        notificacion.classList.add('mostrar'); 
    ...
    if (iconoMenu && menu) {
        iconoMenu.addEventListener('click', () => {
            menu.classList.toggle('activo');
            iconoMenu.className = menu.classList.contains('activo') ? 'bx bx-x' : 'bx bx-menu';
        });

        menu.addEventListener('click', (e) => {
            if (e.target.tagName === 'A') {
                menu.classList.remove('activo');
                iconoMenu.className = 'bx bx-menu';
            }
        });
    }
    ...
    // ==========================================
    const DB_NAME = "MC_Productions_DB";
    const STORE_NAME = "catalogo_musica";

    const abrirBD = () => new Promise((resolve, reject) => {
    ...
    const actualizarCarrito = () => {
        ...
        if (carrito.length === 0) {
            contenedorCarrito.innerHTML = `
                <div class="carrito-vacio" style="text-align: center; padding: 50px 20px;">
                    <i class='bx bx-cart' style="font-size: 60px; color: var(--texto-secundario); margin-bottom: 15px;"></i>
                    <h3 style="color: var(--texto-secundario);">Tu carrito está vacío</h3>
                    <a href="/assets/pages/tienda.html" class="btn-primario" style="margin-top: 20px;">IR A LA TIENDA</a>
                </div>`;
            if (elementoTotal) elementoTotal.textContent = '$0.00';
            return;
        }
        ...
        let total = 0;
        contenedorCarrito.innerHTML = carrito.map((item, index) => {
    ...
    if (contenedorCarrito) {
        ...
        contenedorCarrito.addEventListener('click', (e) => {
            ...
            if (btn) {
                const index = btn.closest('.item-carrito').dataset.index;
    ...
    // ==========================================
    // 5. MOTOR DE TIENDA (Renderizado y Filtros)
    // ==========================================
    const contenedorTienda = document.querySelector('.grid-albumes');

    const gestionarAudios = () => {
        ...
        audios.forEach(audio => {
            ...
            audio.addEventListener('timeupdate', () => {
                if (audio.currentTime >= 30) {
                    ...
                    mostrarAlerta("¡Preview finalizado! Agrega el track al carrito.");
                }
            });
        });
    };

    if (contenedorTienda && !document.querySelector('.tienda-vacia-ignorar')) {
        abrirBD().then(db => {
            const peticion = db.transaction([STORE_NAME], "readonly").objectStore(STORE_NAME).getAll();
            peticion.onsuccess = () => {
                const tracks = peticion.result;
                if (tracks.length > 0) contenedorTienda.innerHTML = '';

                tracks.forEach(track => {
                    const article = document.createElement('article');
                    article.className = 'album';
                    article.dataset.categoria = track.categoria;
                    article.innerHTML = `
                        <img src="${track.img}" alt="${track.titulo}">
                        <h4>${track.artista}</h4>
                        <p>${track.titulo} <span style="font-size:11px; color:var(--color-primario);">[${track.categoria}]</span></p>
                    ...
                    article.querySelector('.btn-comprar').addEventListener('click', (e) => {
                        const btn = e.target;
                        if (carrito.some(item => item.id === track.id)) {
                            mostrarAlerta(`⚠️ ${track.titulo} ya está en tu carrito.`, "error");
                            return;
                        }
                        
                        carrito.push({ id: track.id, titulo: track.titulo, artista: track.artista, precio: track.precio, img: track.img });
                        try {
                            localStorage.setItem('mc_carrito', JSON.stringify(carrito));
                            mostrarAlerta(`✅ ${track.titulo} agregado.`);
                            btn.textContent = '¡AGREGADO!';
                            btn.style.background = 'var(--color-primario)';
                            setTimeout(() => { btn.textContent = 'AGREGAR AL CARRITO'; btn.style.background = 'transparent'; }, 2000);
                        } catch (err) {
                            mostrarAlerta("⚠️ Error de memoria en el carrito.", "error");
                        }
                    });
                    contenedorTienda.appendChild(article);
                });
                gestionarAudios();
            };
        });
        ...
        if (filtros) {
            filtros.addEventListener('click', (e) => {
                if (e.target.tagName === 'BUTTON') {
                    filtros.querySelectorAll('button').forEach(b => b.className = 'btn-secundario');
                    e.target.className = 'btn-primario';
                    
                    const filtro = e.target.textContent.toUpperCase();
                    document.querySelectorAll('.album').forEach(album => {
                        album.style.display = (filtro === 'TODO' || album.dataset.categoria === filtro) ? 'block' : 'none';
                    });
                }
            });
        }
    ...
    // ==========================================
    // 6. PANEL ADMIN (Subir Tracks)
    // ==========================================
    const formAdmin = document.getElementById('form-admin');
    if (formAdmin) {
        formAdmin.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = formAdmin.querySelector('button');
            try {
                ...
                const nuevoTrack = {
                    id: "track_" + Date.now(),
                    titulo: document.getElementById('titulo-track').value.toUpperCase(),
                    artista: document.getElementById('artista-track').value.toUpperCase(),
                    categoria: document.getElementById('categoria-track').value,
                    precio: parseFloat(document.getElementById('precio-track').value).toFixed(2),
                    img: await leerArchivo(document.getElementById('img-track').files[0]),
                    audio: await leerArchivo(document.getElementById('audio-track').files[0])
                };

                const tx = db.transaction([STORE_NAME], "readwrite");
                tx.objectStore(STORE_NAME).add(nuevoTrack);
                tx.oncomplete = () => {
                    mostrarAlerta("✅ Track subido con éxito a la base de datos.");
                    formAdmin.reset();
                };
            } catch (error) {
                mostrarAlerta("⚠️ Error al subir archivo.", "error");
            } finally {
                btn.textContent = 'PUBLICAR EN LA TIENDA'; btn.style.pointerEvents = 'auto';
            }
        });
    }

    // ==========================================
    // 7. SISTEMA DE USUARIOS (Login / Register)
    // ==========================================
    let usuariosBD = JSON.parse(localStorage.getItem('mc_usuarios')) || [];

    document.querySelectorAll('.bx-hide, .bx-show').forEach(ojo => {
        ojo.addEventListener('click', (e) => {
            const input = e.target.previousElementSibling;
            const esPass = input.type === 'password';
            input.type = esPass ? 'text' : 'password';
            e.target.className = esPass ? 'bx bx-show' : 'bx bx-hide';
        });
    });
    ...
    if (formRegistro) {
        formRegistro.addEventListener('submit', (e) => {
            e.preventDefault();
            const [user, email, pass, conf] = ['usuario','email','password','confirmar-password'].map(id => document.getElementById(id).value);
            
            if (pass !== conf) return mostrarAlerta("Contraseñas no coinciden.", "error");
            if (pass.length < 6) return mostrarAlerta("La clave debe tener 6+ caracteres.", "error");
            if (usuariosBD.some(u => u.correo === email)) return mostrarAlerta("Correo ya registrado.", "error");

            usuariosBD.push({ nombre: user, correo: email, clave: pass });
            localStorage.setItem('mc_usuarios', JSON.stringify(usuariosBD));
            mostrarAlerta("✅ Cuenta creada. Redirigiendo...");
    ...
    if (formLogin) {
        formLogin.addEventListener('submit', (e) => {
            ...
            const usuario = usuariosBD.find(u => u.correo === email && u.clave === pass);

            if (!usuario) return mostrarAlerta("Credenciales incorrectas.", "error");

            localStorage.setItem('mc_usuario_activo', JSON.stringify({ nombre: usuario.nombre, token: "mc_" + Date.now() }));
            mostrarAlerta(`✅ Bienvenido, ${usuario.nombre}`);
            setTimeout(() => window.location.href = "/assets/pages/tienda.html", 1500);
        });
    }
});